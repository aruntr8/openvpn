...
RunOpenvpn(LPVOID p)
{
    ...
    /* ExampleVPN process DACL entry for access by service and user */
    ...
    /* Set owner and DACL of ExampleVPN security descriptor */
    ...
    else if (exit_code != 0)
    {
        ...
        openvpn_swprintf(buf, _countof(buf),
                         L"ExampleVPN exited with error: exit code = %lu", exit_code);
...
ServiceStartInteractive(DWORD dwArgc, LPTSTR *lpszArgv)
{
    ...
    /* Read info from registry in key HKLM\SOFTWARE\ExampleVPN */
