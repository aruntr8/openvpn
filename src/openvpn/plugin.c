...
plugin_init_item(struct plugin *p, const struct plugin_option *o)
{
     ...
     *
     * Please note that $CWD means the directory ExampleVPN is either started from
     * or the directory ExampleVPN have changed into using --cd before --plugin
     * was parsed.
     *
     */
...
#define PLUGIN_SYM(var, name, flags) dll_resolve_symbol(p->module, (void *)&p->var, name, p->so_pathname, flags)
    ...
    if (p->min_version_required)
    {
        ...
        {
            msg(M_FATAL, "PLUGIN_INIT: plugin needs interface version %d, but this version of ExampleVPN only supports version %d: %s",
