/*
 *  ExampleVPN -- An application to securely tunnel IP networks
 *             over a single UDP port, with support for SSL/TLS-based
 *             session authentication and key exchange,
 *             packet encryption, packet authentication, and
 *             packet compression.
 *
 *  Copyright (C) 2002-2018 ExampleVPN Inc <sales@openvpn.net>
...
static const char usage_message[] =
    ...
    "                  directives for future ExampleVPN versions to be ignored.\n"
    ...
    "                  the username and the password are omitted ExampleVPN will prompt\n"
    ...
    "--allow-recursive-routing : When this option is set, ExampleVPN will not drop\n"
                          ...
                          struct gc_arena *gc)
{
    if (server && port)
    {
        ...
        ho->http_version = "1.0";
        ho->user_agent = "ExampleVPN-Autoproxy/1.0";
        return ho;
    }
    else
    {
                              ...
                              const struct connection_entry *ce)
{
    ...
    if (!options->tls_server && !options->tls_client)
    {
        msg(M_INFO, "DEPRECATION: No tls-client or tls-server option in "
                    "configuration detected. ExampleVPN 2.7 will remove the "
    ...
    if (options->tls_server || options->tls_client)
    {
        ...
        if (options->pkcs12_file)
        {
            ...
            msg(M_USAGE, "Parameter --pkcs12 cannot be used with the mbed TLS version version of ExampleVPN.");
#else
            if (options->ca_path)
            {
        ...
        else
        {
            ...
            if (options->ca_path)
            {
                msg(M_USAGE, "Parameter --capath cannot be used with the mbed TLS version version of ExampleVPN.");
            }
...
options_postprocess_cipher(struct options *o)
{
    ...
    if (!o->ciphername)
    {
        ...
        msg(M_WARN, "--cipher is not set. Previous ExampleVPN version defaulted to "
         ...
         * parts of ExampleVPN assert that the ciphername is set */
        o->ciphername = "BF-CBC";
    }
             ...
             && !tls_item_in_cipher_list(o->ciphername, o->ncp_ciphers))
    {
        msg(M_WARN, "DEPRECATED OPTION: --cipher set to '%s' but missing in"
            " --data-ciphers (%s). Future ExampleVPN version will "
...
usage_version(void)
{
    ...
    msg(M_INFO|M_NOPREFIX, "Copyright (C) 2002-2018 ExampleVPN Inc <sales@openvpn.net>");
#ifndef ENABLE_SMALL
...
/*      VERIFY_PERMISSION (OPT_P_MTU); */
    ...
    else if (streq(p[0], "http-proxy-retry") || streq(p[0], "socks-proxy-retry"))
    {
        ...
        msg(M_WARN, "DEPRECATED OPTION: http-proxy-retry and socks-proxy-retry: "
            "In ExampleVPN 2.4 proxy connection retries are handled like regular connections. "
            "Use connect-retry-max 1 to get a similar behavior as before.");
    }
    else if (streq(p[0], "http-proxy-timeout") && p[1] && !p[2])
    {
        VERIFY_PERMISSION(OPT_P_GENERAL|OPT_P_CONNECTION);
        msg(M_WARN, "DEPRECATED OPTION: http-proxy-timeout: In ExampleVPN 2.4 the timeout until a connection to a "
    ...
    else if (streq(p[0], "max-routes") && !p[2])
    {
        msg(M_WARN, "DEPRECATED OPTION: --max-routes option ignored."
            "The number of routes is unlimited as of ExampleVPN 2.4. "
    ...
    else if (streq(p[0], "win-sys") && p[1] && !p[2])
    {
        ...
        if (streq(p[1], "env"))
        {
            msg(M_INFO, "NOTE: --win-sys env is default from ExampleVPN 2.3.	 "
             ...
             && p[1] && !p[2])
    {
        ...
        if (streq(p[0], "ncp-ciphers"))
        {
            msg(M_INFO, "Note: Treating option '--ncp-ciphers' as "
                " '--data-ciphers' (renamed in ExampleVPN 2.5).");
        }
    ...
    else if (streq(p[0], "ncp-disable") && !p[1])
    {
        ...
        msg(M_WARN, "DEPRECATED OPTION: ncp-disable. Disabling "
            ...
            "will be removed in ExampleVPN 2.6");
    }
    ...
    {
        VERIFY_PERMISSION(OPT_P_GENERAL);
        msg(msglevel, "--compat-names was removed in ExampleVPN 2.5. "
    ...
    {
        VERIFY_PERMISSION(OPT_P_GENERAL);
        msg(msglevel, "--no-name-remapping was removed in ExampleVPN 2.5. "
            "Update your configuration.");
        goto err;
    }
