...
static WCHAR *FIREWALL_NAME = L"ExampleVPN";
                      ...
                      )
{
    ...
    *engine_handle = NULL;
    ...
    /* First filter. Permit IPv4 DNS queries from ExampleVPN itself. */
    ...
    CHECK_ERROR(err, "Add filter to permit IPv4 port 53 traffic from ExampleVPN failed");

    /* Second filter. Permit IPv6 DNS queries from ExampleVPN itself. */
    Filter.layerKey = FWPM_LAYER_ALE_AUTH_CONNECT_V6;

    err = FwpmFilterAdd0(*engine_handle, &Filter, NULL, &filterid);
    CHECK_ERROR(err, "Add filter to permit IPv6 port 53 traffic from ExampleVPN failed");
