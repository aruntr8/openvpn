...
/** @name Function for authenticating a new connection from a remote ExampleVPN peer
 *  @{ */

/**
 * Verify that the remote ExampleVPN peer's certificate allows setting up a
 * VPN tunnel.
 * @ingroup control_tls
 *
 * This callback function is called when a new TLS session is being setup to
 * determine whether the remote ExampleVPN peer's certificate is allowed to
 ...
 * it calls the \c verify_cert() function, which performs ExampleVPN-specific
 * verification.
 *
 * @param session_obj  - The ExampleVPN \c tls_session associated with this object,
 ...
 * @param flags        - Whether the remote ExampleVPN peer's certificate
