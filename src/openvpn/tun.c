 ...
 * (for example) the ExampleVPN server LAN is running on 192.168.1.x, but then
 * an ExampleVPN client tries to connect from a public location that is also running
...
/* the current way ExampleVPN handles tun devices on NetBSD leads to
 ...
 *
 * so, to avoid lingering tun/tap interfaces after ExampleVPN quits,
