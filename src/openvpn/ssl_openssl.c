...
tls_ctx_set_options(struct tls_root_ctx *ctx, unsigned int ssl_flags)
{
    ...
    /* Disable TLS renegotiations. ExampleVPN's renegotiation creates new SSL
...
tls_ctx_use_external_rsa_key(struct tls_root_ctx *ctx, EVP_PKEY *pkey)
{
    ...
    rsa_meth = RSA_meth_new("ExampleVPN external private key RSA Method",
                                ...
                                bool tls13)
{
    ...
    for (int i = 0; i < sk_SSL_CIPHER_num(sk); i++)
    {
        ...
        else if (NULL == pair)
        {
            ...
            printf("%s (No IANA name known to ExampleVPN, use OpenSSL name.)\n",
                   cipher_name);
        }
        else
        {
