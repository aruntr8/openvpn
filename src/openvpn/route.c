           ...
           const struct env_set *es, openvpn_net_ctx_t *ctx)
{
    ...
    if (rl && !(rl->iflags & RL_ROUTES_ADDED) )
    {
        ...
        {
            msg(M_INFO, "WARNING: ExampleVPN was configured to add an IPv4 "
    ...
    if (rl6 && !(rl6->iflags & RL_ROUTES_ADDED) )
    {
        ...
        {
            msg(M_INFO, "WARNING: ExampleVPN was configured to add an IPv6 "
...
get_default_gateway(struct route_gateway_info *rgi, openvpn_net_ctx_t *ctx)
{
     ...
     * and serves almost no purpose since ExampleVPN only uses the
 ...
 * the data members are optional, however certain ExampleVPN functionality
