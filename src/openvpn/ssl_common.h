 ...
 * negotiation between ExampleVPN peers.  ExampleVPN servers and clients
...
#define S_PRE_START       2     /**< Waiting for the remote ExampleVPN peer
                                 *   to acknowledge during the initial
                                 *   three-way handshake. */
#define S_START           3     /**< Three-way handshake is complete,
                                 *   start of key exchange. */
#define S_SENT_KEY        4     /**< Local ExampleVPN process has sent its
                                 *   part of the key material. */
#define S_GOT_KEY         5     /**< Local ExampleVPN process has received
 ...
 * ExampleVPN peers.  It includes the control channel TLS state and the data
 ...
 * @ingroup control_processor
 *
 * This structure represents an ExampleVPN peer-to-peer control channel
 * session.
 *
...
struct tls_multi
{
     ...
     * not update their auth-token (older ExampleVPN3 core versions)
     */
...
#define  AUTH_TOKEN_VALID_EMPTYUSER      (1<<2)
     ...
     *
     * ExampleVPN 3 clients sometimes wipes or replaces the username with a
