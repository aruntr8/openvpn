...
/*
 * A foreign (non-ExampleVPN) connection we are proxying,
 * usually HTTPS
 */
...
/*
 * Called from the main ExampleVPN process to enable the port
 * share proxy.
 */
struct port_share *
 ...
 * data payload, return true if the protocol is that of an ExampleVPN
 * client attempting to connect with an ExampleVPN server.
 */
bool
