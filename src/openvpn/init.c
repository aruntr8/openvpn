...
#ifdef ENABLE_SYSTEMD
    /* We can report the PID via getpid() to systemd here as ExampleVPN will not
...
do_uid_gid_chroot(struct context *c, bool no_delay)
{
    ...
    if (c0 && !c0->uid_gid_chroot_set)
    {
        ...
        /* Apply a SELinux context in order to restrict what ExampleVPN can do
 ...
 * parts of ExampleVPN might want to fill the route-list with info, e.g. DHCP)
 */
static void
 ...
 * time ExampleVPN would wait without management
 */
static bool
...
/*
 * Initialize the persistent component of ExampleVPN's TLS mode,
...
do_init_crypto_tls(struct context *c, const unsigned int flags)
{
    ...
    /*
     * Initialize ExampleVPN's master TLS-mode object.
     */
...
#if PORT_SHARE
    /* share ExampleVPN port with foreign (such as HTTPS) server */
