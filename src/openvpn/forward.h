 ...
 * will be sent through a VPN tunnel to a remote ExampleVPN peer.  It's
 ...
 *   - Call \c tls_post_encrypt() to prepend the one-byte ExampleVPN header
 ...
 *
 * ExampleVPN running as client or as UDP server only has a single external
 * network socket, so this function can be called with the single (client
 * mode) or top level (UDP server) context as its argument. ExampleVPN
