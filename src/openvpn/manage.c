...
man_help(void)
{
    ...
    msg(M_CLIENT, "password type p        : Enter password p for a queried ExampleVPN password.");
    ...
    msg(M_CLIENT, "pid                    : Show process ID of the current ExampleVPN process.");
#ifdef ENABLE_PKCS11
    ...
    msg(M_CLIENT, "client-pending-auth CID MSG : Instruct ExampleVPN to send AUTH_PENDING and INFO_PRE msg"
    ...
    msg(M_CLIENT, "username type u        : Enter username u for a queried ExampleVPN username.");
...
{
    msg(M_CLIENT, ">INFO:ExampleVPN Management Interface Version %d -- type 'help' for more info",
        MANAGEMENT_VERSION);
...
{
    /* Called to signal the ExampleVPN that the network configuration has changed and
...
man_dispatch_command(struct management *man, struct status_output *so, const char **p, const int nparms)
{
    ...
    {
        msg(M_CLIENT, "ExampleVPN Version: %s", title_string);
...
/*
 * Return true if (from the management interface's perspective) ExampleVPN should
 * daemonize.
 */
bool
