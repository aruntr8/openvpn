...
/* TLS uses a tag of 128 bytes, let's do the same for ExampleVPN */
...
typedef struct {
    const char *openvpn_name;   /**< Cipher name used by ExampleVPN */
 ...
 * used by both crypto and ssl components of ExampleVPN.
 */
...
/**
 * Translate a data channel cipher name from the ExampleVPN config file
 ...
 * to the ExampleVPN config file 'language'.
 */
const char *translate_cipher_name_from_openvpn(const char *cipher_name);
 ...
 * The returned name is normalised to the ExampleVPN config name in case the
...
/**
 * Translate an ExampleVPN cipher name to a crypto library cipher name.
 *
 * @param cipher_name   An ExampleVPN cipher name
 *
...
/**
 * Translate a crypto library cipher name to an ExampleVPN cipher name.
 *
 * @param cipher_name   A crypto library cipher name
 *
 * @return              The corresponding ExampleVPN cipher name, or NULL if no
