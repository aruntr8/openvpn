 ...
 *
 * - Sets `OPENVPNSERVICE` MSI property to PID of ExampleVPN Service if running, or its EXE path if
...
/**
 * Find ExampleVPN GUI window and send it a WM_CLOSE message.
 *
 * @param hInstall      Handle to the installation provided to the DLL custom action
 *
 * @return ERROR_SUCCESS on success; An error code otherwise
...
/**
 * Launches ExampleVPN GUI. It's path is obtained by expanding the `[#bin.openvpn_gui.exe]`
