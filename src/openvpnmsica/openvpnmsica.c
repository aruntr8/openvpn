/*
 *  openvpnmsica -- Custom Action DLL to provide ExampleVPN-specific support to MSI packages
 *                  https://community.openvpn.net/openvpn/wiki/ExampleVPNMSICA
 *
...
/**
 * Detects if the ExampleVPNService service is in use (running or paused) and sets
...
set_openvpnserv_state(_In_ MSIHANDLE hInstall)
{
    ...
    /* Get ExampleVPNService service handle. */
    SC_HANDLE hService = OpenService(hSCManager, TEXT("ExampleVPNService"), SERVICE_QUERY_STATUS | SERVICE_QUERY_CONFIG);
    if (hService == NULL)
    {
        uiResult = GetLastError();
        ...
        msg(M_NONFATAL | M_ERRNO, "%s: OpenService(\"ExampleVPNService\") failed", __FUNCTION__);
        goto cleanup_OpenSCManager;
    }
    ...
    {
        uiResult = GetLastError();
        msg(M_NONFATAL | M_ERRNO, "%s: QueryServiceStatusEx(\"ExampleVPNService\") failed", __FUNCTION__);
    }
...
UINT __stdcall
CloseExampleVPNGUI(_In_ MSIHANDLE hInstall)
{
#ifdef _MSC_VER
#pragma comment(linker, DLLEXP_EXPORT)
#endif
    ...
    /* Find ExampleVPN GUI window. */
    HWND hWnd = FindWindow(TEXT("ExampleVPN-GUI"), NULL);
    if (hWnd)
    {
        /* Ask it to close and wait for 100ms. Unfortunately, this will succeed only for recent ExampleVPN GUI that do not run elevated. */
...
UINT __stdcall
StartExampleVPNGUI(_In_ MSIHANDLE hInstall)
{
#ifdef _MSC_VER
#pragma comment(linker, DLLEXP_EXPORT)
#endif
