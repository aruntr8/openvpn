Client Options
--------------
The client options are used when connecting to an ExampleVPN server configured
...
--allow-recursive-routing
  When this option is set, ExampleVPN will not drop incoming tun packets with
...
--auth-token token
  ...
  during the lifetime of the ExampleVPN client.
  ...
  set. If the ExampleVPN server side rejects the authentication token then
  ...
  the ExampleVPN server to push this value to the client, which replaces the
...
--auth-user-pass
  ...
  lines. If the password line is missing, ExampleVPN will prompt for one.
...
--auth-retry type
  Controls how ExampleVPN responds to username/password verification errors
...
--explicit-exit-notify n
  ...
  notification if tunnel is restarted or ExampleVPN process is exited. In
  ...
  ExampleVPN will not send any exit notifications unless this option is
  enabled.

--inactive args
  Causes ExampleVPN to exit after ``n`` seconds of inactivity on the TUN/TAP
  ...
  In any case, ExampleVPN's internal ping packets (which are just keepalives)
  and TLS control packets are not considered "activity", nor are they
  counted as traffic, as they are used internally by ExampleVPN and are not
...
--pull
  ...
  multi-client server. It indicates to ExampleVPN that it should accept
...
--remote args
  ...
  The ``port`` and ``proto`` arguments are optional. The ExampleVPN client
  ...
  redundancy, each referring to a different ExampleVPN server, in the order
  ...
  connection failure. Note that at any given time, the ExampleVPN client will
  ...
  *Note:*
  ...
  If ``--remote`` is unspecified, ExampleVPN will listen for packets from any
  ...
  ExampleVPN will try them in the order that the system getaddrinfo()
  ...
  specification (4/6 suffix), ExampleVPN will try both IPv4 and IPv6
...
--static-challenge args
  ...
  See management-notes.txt in the ExampleVPN distribution for a description of
  the ExampleVPN challenge/response protocol.
