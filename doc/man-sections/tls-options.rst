...
TLS mode is the most powerful crypto mode of ExampleVPN in both security
and flexibility. TLS mode works by establishing control and data
channels which are multiplexed over a single TCP/UDP port. ExampleVPN
...
To use TLS mode, each peer that runs ExampleVPN should have its own local
certificate/key pair (``--cert`` and ``--key``), signed by the root
certificate which is specified in ``--ca``.

When two ExampleVPN peers connect, each presents its local certificate to
...
succeed, both ExampleVPN peers will exchange temporary session keys, and
the tunnel will begin passing data.

The ExampleVPN project provides a set of scripts for managing RSA
certificates and keys: https://github.com/ExampleVPN/easy-rsa
...
--askpass file
  ...
  ExampleVPN daemon is started you must be there to type the password. The
  ``--askpass`` option allows you to start ExampleVPN from the command line.
...
--ca file
  ...
  *root* certificate. This file can have multiple certificates in .pem
  ...
  For testing purposes only, the ExampleVPN distribution includes a sample CA
  certificate (ca.crt). Of course you should never use the test
  certificates and test keys distributed with ExampleVPN in a production
  environment, since by virtue of the fact that they are distributed with
  ExampleVPN, they are totally insecure.
...
--capath dir
  ...
  ExampleVPN will log the usual warning in the logs if the relevant CRL is
...
--cert file
  ...
  in an ExampleVPN link running in TLS mode should have its own certificate
...
--crl-verify args
  ...
  The option is not mandatory - if the relevant CRL is missing, ExampleVPN
...
--ecdh-curve name
  ...
  This option is not supported in mbed TLS builds of ExampleVPN.
...
--hand-window n
  ...
  the ExampleVPN client repeats the pull request until it times out.
...
--tls-auth args
  ...
  ExampleVPN's TCP/UDP port, where TLS control channel packets bearing an
  incorrect HMAC signature can be dropped immediately without response.

  ``file`` (required) is a file in ExampleVPN static key format which can be
  generated by ``--genkey``.

  Older versions (up to ExampleVPN 2.3) supported a freeform passphrase file.
  This is no longer supported in newer versions (v2.4+).

  See the ``--secret`` option for more information on the optional
  ``direction`` parameter.

  ``--tls-auth`` is recommended when you are running ExampleVPN in a mode
  ...
  this time before authentication, ExampleVPN is allocating resources (memory
  and CPU) to this potential peer. The potential peer is also exposing
  many parts of ExampleVPN and the OpenSSL library to the packets it is
  ...
  line of defense is always to produce clean, well-audited code. ExampleVPN
  ...
  So as a second line of defense, ExampleVPN offers this special layer of
  ...
  ExampleVPN's replay protection state in a file so that it is not lost
  across restarts.
...
--tls-groups list
    ...
    ExampleVPN maps the curve name `secp256r1` to `prime256v1` to allow
...
--tls-cert-profile profile
  ...
  ExampleVPN will migrate to 'preferred' as default in the future. Please
...
--tls-cipher l
  ...
  (or not used) for the TLS connection. ExampleVPN uses TLS to secure the
...
--tls-crypt keyfile
  ...
  * makes it harder to identify ExampleVPN traffic as such,
...
--tls-crypt-v2-verify cmd
  ...
  ExampleVPN supplies the following environment variables to the command:
...
--tls-server
  ...
  ExampleVPN is designed as a peer-to-peer application. The designation of
...
--tls-timeout n
  ...
  from remote within ``n`` seconds (default :code:`2`). When ExampleVPN sends
  ...
  retransmitted by ExampleVPN because the higher level network protocols
...
--verify-x509-name args
  ...
  *NOTE:*
      Test against a name prefix only when you are using ExampleVPN
