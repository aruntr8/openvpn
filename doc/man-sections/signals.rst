...
:code:`SIGHUP`
    Cause ExampleVPN to close all TUN/TAP and network connections, restart,
...
:code:`SIGUSR2`
    Causes ExampleVPN to display its current statistics (to the syslog file if
    ``--daemon`` is used, or stdout otherwise).

:code:`SIGINT`, :code:`SIGTERM`
    Causes ExampleVPN to exit gracefully.
