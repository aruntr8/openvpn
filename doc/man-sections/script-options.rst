...
ExampleVPN can execute external scripts in various phases of the lifetime of
the ExampleVPN process.
...
#. ``--learn-address``
   ...
   address is added to ExampleVPN's internal routing table.
...
--auth-user-pass-verify args
  ...
  ExampleVPN will run command ``cmd`` to validate the username/password
  ...
  If ``method`` is set to :code:`via-env`, ExampleVPN will call ``script``
  ...
  If ``method`` is set to :code:`via-file`, ExampleVPN will write the username
  ...
  automatically deleted by ExampleVPN after the script returns. The location
  ...
  extending ExampleVPN's authentication capabilities.
  ...
  :code:`sample-scripts/auth-pam.pl` in the ExampleVPN source distribution.
...
--client-connect cmd
  ...
  generated config file directives back to ExampleVPN.
...
--ipchange cmd
  ...
  deliver a ``SIGHUP`` or ``SIGUSR1`` signal to ExampleVPN. ExampleVPN will
...
--learn-address cmd
  ...
  :code:`$1` - [operation]
      ...
      ExampleVPN's internal routing table.
  ...
  a failure code (non-zero), ExampleVPN will reject the address and will not
  ...
  ExampleVPN provides the association between virtual IP or MAC address and
...
--setenv args
  ...
  directive might be valid in future ExampleVPN versions.
  ...
  Versions prior to ExampleVPN 2.3.3 will always ignore options set with the
...
--tls-verify cmd
  ...
  certificate, see the file ``verify-cn`` in the ExampleVPN distribution.
...
--up cmd
  ...
  Note that if ``cmd`` includes arguments, all ExampleVPN-generated arguments
  ...
  partial reinitialization of ExampleVPN where the TUN/TAP instance is
  ...
  *NOTE:*
     On restart, ExampleVPN will not pass the full set of environment
  ...
  Note that ExampleVPN also provides the ``--ifconfig`` option to
  ...
  If ``--ifconfig`` is also specified, ExampleVPN will pass the ifconfig
...
In certain cases, ExampleVPN will perform remapping of characters in
...
Here is a brief rundown of ExampleVPN's current string types and the
...
*--auth-user-pass username*
   ...
   starting with ExampleVPN 2.0.1, the username is passed to the
...
As of ExampleVPN 2.0-beta12, in server mode, environmental variables set by
ExampleVPN are scoped according to the client objects they are associated
...
:code:`ifconfig_broadcast`
    ...
    ExampleVPN calling the :code:`ifconfig` or :code:`netsh` (windows version
...
:code:`ifconfig_ipv6_local`
    ...
    ``--ifconfig-ipv6`` option (first parameter). Set prior to ExampleVPN
...
:code:`ifconfig_ipv6_netbits`
    ...
    option (first parameter). Set prior to ExampleVPN calling the
...
:code:`ifconfig_ipv6_remote`
    ...
    ``--ifconfig-ipv6`` option (second parameter). Set prior to ExampleVPN
    calling the :code:`ifconfig` or :code:`netsh` (windows version of
    ifconfig) commands which normally occurs prior to ``--up`` script
    execution.
...
:code:`ifconfig_netmask`
    ...
    used. Set prior to ExampleVPN calling the :code:`ifconfig` or
...
:code:`route_{parm}_{n}`
    ...
    ``n`` is the ExampleVPN route number, starting from 1.
...
:code:`X509_{n}_{subject_field}`
    ...
    following variables would be set on the ExampleVPN server using the sample
    ...
    ::
       ...
       X509_0_O=ExampleVPN-TEST
       X509_0_ST=NA
       X509_0_C=KG
       X509_1_emailAddress=me@myhost.mydomain
       X509_1_O=ExampleVPN-TEST
       X509_1_L=BISHKEK
       X509_1_ST=NA
       X509_1_C=KG
