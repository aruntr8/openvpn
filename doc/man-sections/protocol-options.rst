Protocol options
----------------
Options in this section affect features available in the ExampleVPN wire
protocol.  Many of these options also define the encryption options
of the data channel in the ExampleVPN wire protocol.  These options must be
...
--allow-compression mode
  ...
  ExampleVPN when compression is used and allowed.
  ...
  :code:`asym`  (default)
      ExampleVPN will only *decompress downlink packets* but *not compress
  ...
  :code:`no`
      ExampleVPN will refuse any non-stub compression.

  :code:`yes`
      ExampleVPN will send and receive compressed packets.
...
--auth alg
  ...
  The ExampleVPN data channel protocol uses encrypt-then-mac (i.e. first
  ...
  ExampleVPN receives a packet with a bad HMAC it will drop the packet. HMAC
...
--cipher alg
  ...
  ExampleVPN 2.4 and newer on both client and server side will automatically
  ...
  To see other ciphers that are available with ExampleVPN, use the
  ``--show-ciphers`` option.
...
--comp-noadapt
  **DEPRECATED** When used in conjunction with ``--comp-lzo``, this option
  will disable ExampleVPN's adaptive compression algorithm. Normally, adaptive
  ...
  transfer of a large, compressed file. With adaptive compression, ExampleVPN
...
--keysize n
  **DEPRECATED** This option will be removed in ExampleVPN 2.6.
...
--data-ciphers cipher-list
  ...
  ExampleVPN will inherit the cipher of the peer if that cipher is different
  ...
  Note for using NCP with an ExampleVPN 2.4 peer: This list must include the
  :code:`AES-256-GCM` and :code:`AES-128-GCM` ciphers.

  This list is restricted to be 127 chars long after conversion to ExampleVPN
  ciphers.

  This option was called ``--ncp-ciphers`` in ExampleVPN 2.4 but has been renamed
  to ``--data-ciphers`` in ExampleVPN 2.5 to more accurately reflect its meaning.
...
--data-ciphers-fallback alg
    ...
    connect to peers that are running ExampleVPN 2.3 and older version, and
...
--secret args
  **DEPRECATED** Enable Static Key encryption mode (non-TLS). Use pre-shared secret
  ...
  key. While pre-1.5 versions of ExampleVPN generate 1024 bit key files, any
  version of ExampleVPN which supports the ``direction`` parameter, will also
  ...
  mark the ciphertext packets as being generated by ExampleVPN. Anyone
