...
--show-tls
  ...
  ExampleVPN uses TLS to secure the control channel, over which the keys that
...
--genkey args
  ...
  :code:`secret`                Standard ExampleVPN shared secret keys
  ...
  * Generating *TLS Crypt v2 Server key*
    Generate a ``--tls-crypt-v2`` key to be used by an ExampleVPN server.
  ...
  * Generating *TLS Crypt v2 Client key*
    Generate a --tls-crypt-v2 key to be used by ExampleVPN clients.  The
    ...
    If no metadata is supplied, ExampleVPN will use a 64-bit unix timestamp
  ...
  * Generate *Authentication Token key*
    ...
    *Note:*
       ...
       ExampleVPN server will accept as valid.
