...
--block-ipv6
  ...
  traffic to ExampleVPN and answer all requests with no route to host,
...
--dev-node node
  ...
  :code:`/dev/tun`, :code:`/dev/tap`, etc. If ExampleVPN cannot figure out
...
--dhcp-option args
  ...
  handled by the ``tap-windows6`` driver by default or directly by ExampleVPN
  if dhcp is disabled or the ``wintun`` driver is in use. The
  ``ExampleVPN for Android`` client also handles them internally.
  ...
  :code: `PROXY_HTTP` ``host`` ``port``
        ...
        This option currently only works on ExampleVPN for Android and requires
        Android 10 or later.
...
--redirect-gateway flags
  ...
  :code:`local`
      Add the :code:`local` flag if both ExampleVPN peers are directly
...
--route args
  ...
  across ExampleVPN's platform space.
  ...
  :code:`remote_host`
      The ``--remote`` address if ExampleVPN is being run in
...
--route-gateway arg
  ...
  be extracted from a DHCP negotiation with the ExampleVPN server-side LAN.
...
--topology mode
  ...
  :code:`subnet`
    ...
    ExampleVPN 2.1 or higher, or ExampleVPN 2.0.x which has been manually patched
...
--tun-mtu n
  ...
  ExampleVPN requires that packets on the control and data channels be sent
  unfragmented.
...
--tun-mtu-extra n
  ...
  when TAP devices are used. This parameter only controls internal ExampleVPN
...
--mktun
  ...
  through multiple instantiations of ExampleVPN and die only when they are
  ...
  tunnel will not be reset if the ExampleVPN peer restarts. This can be
