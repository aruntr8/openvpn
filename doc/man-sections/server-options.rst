Server Options
--------------
Starting with ExampleVPN 2.0, a multi-client TCP/UDP server mode is
supported, and can be enabled with the ``--mode server`` option. In
server mode, ExampleVPN will listen on a single port for incoming client
...
--auth-gen-token args
  ...
  After successful user/password authentication, the ExampleVPN server will
  with this option generate a temporary authentication token and push that
  to the client. On the following renegotiations, the ExampleVPN client will pass
  ...
  :code:`AuthenticatedEmptyUser` / :code:`ExpiredEmptyUser`
      ...
      ExampleVPN 3.  If this workaround is not needed these two cases should
...
--auth-gen-token-secret file
  ...
  ``--auth-gen-token`` If ``file`` is not present ExampleVPN will generate a
  ...
  between multiple ExampleVPN servers with their auth-token.

--auth-user-pass-optional
  Allow connections by clients that do not specify a username/password.
  Normally, when ``--auth-user-pass-verify`` or
  ``--management-client-auth`` are specified (or an authentication plugin
  module), the ExampleVPN server daemon will require connecting clients to
...
--client-config-dir dir
  ...
  connecting client has been authenticated, ExampleVPN will look in this
  ...
  ExampleVPN will instead try to open and parse a default file called
  "DEFAULT", which may be provided but is not required. Note that the
  configuration files must be readable by the ExampleVPN process after it has
...
--client-to-client
  Because the ExampleVPN server mode handles multiple clients through a
  single tun or tap interface, it is effectively a router. The
  ``--client-to-client`` flag tells ExampleVPN to internally route
...
--duplicate-cn
  ...
  connect. In the absence of this option, ExampleVPN will disconnect a client
...
--ifconfig-pool-persist args
  ...
  Note that the entries in this file are treated by ExampleVPN as
...
--ifconfig-push args
  ...
  Remember also to include a ``--route`` directive in the main ExampleVPN
  config file which encloses ``local``, so that the kernel will know to
  route it to the server's TUN/TAP interface.

  ExampleVPN's internal client IP address selection algorithm works as
  follows:
...
--iroute args
  ...
  from the kernel to ExampleVPN. Once in ExampleVPN, the ``--iroute`` directive
  ...
  this. In order for all clients to see *A*'s subnet, ExampleVPN must push
  this route to all clients EXCEPT for *A*, since the subnet is already
  owned by *A*. ExampleVPN accomplishes this by not not pushing a route to
...
--max-routes-per-client n
  ...
  Note that this directive affects ExampleVPN's internal routing table, not
...
--port-share args
  Share ExampleVPN TCP with another service
  ...
  When run in TCP server mode, share the ExampleVPN port with another
  application, such as an HTTPS server. If ExampleVPN senses a connection to
  its port which is using a non-ExampleVPN protocol, it will proxy the
...
--server args
  A helper directive designed to simplify the configuration of ExampleVPN's
  server mode. This directive will set up an ExampleVPN server which will
...
--server-bridge args
  ...
  the configuration of ExampleVPN's server mode in ethernet bridging
  configurations.
  ...
  DHCP-proxy mode, where connecting ExampleVPN clients will receive an IP
  address for their TAP adapter from the DHCP server running on the
  ExampleVPN server-side LAN. Note that only clients that support the binding
  ...
  ``pool-start-IP`` and ``pool-end-IP``, for ExampleVPN to allocate to
  connecting clients.
...
--verify-client-cert mode
  ...
  but you also specify an ``--auth-user-pass-verify`` script, then ExampleVPN
...
--vlan-tagging
  Server-only option. Turns the ExampleVPN server instance into a switch that
  ...
  The packet filtering takes place in the ExampleVPN server. Clients should
  not have any VLAN tagging configuration applied.

  The ``--vlan-tagging`` option is off by default. While turned off,
  ExampleVPN accepts any Ethernet frame and does not perform any special
