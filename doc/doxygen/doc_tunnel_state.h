 ...
 *
 * This section describes how ExampleVPN stores its VPN tunnel state during
 * operation.
 *
 * ExampleVPN uses several data structures as storage containers for state
 ...
 *
 * Whether an ExampleVPN process is running in client-mode or server-mode
 ...
 *
 * In the old v1.x series, an ExampleVPN process managed only one single VPN
 ...
 *
 * This changed, however, in the v2.x series, as new ExampleVPN versions
 ...
 * majority of these changes are only relevant for ExampleVPN processes
 ...
 *
 * An ExampleVPN process running in client-mode can manage at most one single
 ...
 * is then passed to \c tunnel_point_to_point() which drives ExampleVPN's
 ...
 *
 * An ExampleVPN process running in server-mode can manage multiple
 * simultaneously active VPN tunnels.  For every VPN tunnel active, in
 * other words for every ExampleVPN client which is connected to a server,
 * the ExampleVPN server has one \c context structure in which it stores that
 ...
 *
 * An ExampleVPN process running in server-mode starts in the same \c main()
 ...
 *
 * When an ExampleVPN client makes a new connection to a server, the server
