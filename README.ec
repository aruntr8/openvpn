Since 2.4.0, ExampleVPN has official support for elliptic curve crypto. Elliptic
...
in ExampleVPN; the data channel (encrypting the actual network traffic) uses
symmetric encryption. ECC can be used in TLS for authentication (ECDSA) and key
exchange (ECDH).

Key exchange (ECDH)
-------------------
ExampleVPN 2.4.0 and newer automatically initialize ECDH parameters. When ECDSA is
...
ExampleVPN lets the crypto library decide if possible, or falls back to the
...
An administrator can force an ExampleVPN/OpenSSL server to use a specific curve
...
----------------------
Since ExampleVPN 2.4.0, using ECDSA certificates works 'out of the box'. Which
