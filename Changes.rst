...
Keying Material Exporters (RFC 5705) based key generation
    As part of the cipher negotiation ExampleVPN will automatically prefer
    the RFC5705 based key material generation to the current custom
    ExampleVPN PRF. This feature requires OpenSSL or mbed TLS 2.18+.

Compatibility with OpenSSL in FIPS mode
    ExampleVPN will now work with OpenSSL in FIPS mode. Note, no effort
    has been made to check or implement all the
    requirements/recommendation of FIPS 140-2. This just allows ExampleVPN
...
TLS mode with self-signed certificates
    ...
    become optional. This allows for small ExampleVPN setups without setting up
...
``inetd`` has been removed
    This was a very limited and not-well-tested way to run ExampleVPN, on TCP
    and TAP mode only.
...
ChaCha20-Poly1305 cipher support
    Added support for using the ChaCha20-Poly1305 cipher in the ExampleVPN data
    channel.
...
Improved Data channel cipher negotiation
    ...
    ExampleVPN clients will now signal all supported ciphers from the
    ``data-ciphers`` option to the server via ``IV_CIPHERS``. ExampleVPN
...
Removal of BF-CBC support in default configuration:
    By default ExampleVPN 2.5 will only accept AES-256-GCM and AES-128-GCM as
    data ciphers. ExampleVPN 2.4 allows AES-256-GCM,AES-128-GCM and BF-CBC when
...
Netlink support
    ...
    ``route`` and also enables ExampleVPN to run with less privileges.

    If configured with --enable-iproute2, the ``ip`` command is used
    (as in 2.4).  Support for ``ifconfig`` and ``route`` is gone.

Wintun support
    On Windows, ExampleVPN can now use ``wintun`` devices.  They are faster
...
Improved support for pending authentication
    ...
    disconnecting the ExampleVPN session with AUTH_FAILED. The
...
VLAN support
    ExampleVPN servers in TAP mode can now use 802.1q tagged VLANs
...
- ``inetd`` has been deprecated
  This is a very limited and not-well-tested way to run ExampleVPN, on TCP
  and TAP mode only, which complicates the code quite a bit for little gain.
  To be removed in ExampleVPN 2.6 (unless users protest).

- ``no-iv`` has been removed
  This option was made into a NOOP option with ExampleVPN 2.4.  This has now
...
- plugin-auth-pam now logs via ExampleVPN logging method, no longer to stderr
...
- ``--disable-server`` has been removed from configure (so it is no longer
  possible to build a client-/p2p-only ExampleVPN binary) - the saving in code
...
- ``--disable-crypto`` configure option has been removed.  ExampleVPN is now always
...
Data channel cipher negotiation
    ...
    it requires that ExampleVPN was built without disabling OCC support.
...
Improved Certificate Revocation List (CRL) processing
    ...
    of inside ExampleVPN itself.  The crypto library implementations are more
    strict than the ExampleVPN implementation was.  This might reject peer
    certificates that would previously be accepted.  If this occurs, ExampleVPN
    will log the crypto library's error description.

Dualstack round-robin DNS client connect
    Instead of only using the first address of each ``--remote`` ExampleVPN
...
New improved Windows Background service
    The new ExampleVPNService is based on openvpnserv2, a complete rewrite of the ExampleVPN
    service wrapper. It is intended for launching ExampleVPN instances that should be
    up at all times, instead of being manually launched by a user. ExampleVPNService is
    able to restart individual ExampleVPN processes if they crash, and it also works
    properly on recent Windows versions. ExampleVPNServiceLegacy tends to work poorly,
    if at all, on newer Windows versions (8+) and its use is not recommended.

New interactive Windows service
    The installer starts ExampleVPNServiceInteractive automatically and configures
    it to start	at system startup.

    The interactive Windows service allows unprivileged users to start
    ExampleVPN connections in the global config directory (usually
    C:\\Program Files\\ExampleVPN\\config) using ExampleVPN GUI without any
    extra configuration.

    Users who belong to the built-in Administrator group or to the
    local "ExampleVPN Administrator" group can also store configuration
    files under %USERPROFILE%\\ExampleVPN\\config for use with the
    interactive service.

redirect-gateway ipv6
    ExampleVPN has now feature parity between IPv4 and IPv6 for redirect
...
LZ4 Compression and pushable compression
    Additionally to LZO compression ExampleVPN now also supports LZ4 compression.
...
Authentication tokens
    ...
    ExampleVPN has support for pushing a --auth-token since v2.3.  This option is
    ...
    need to implement this support as well.  In ExampleVPN 2.4 --auth-gen-token
    is introduced, which will allow the ExampleVPN server to generate a random
    token and push it to the client without any changes to the authentication
    modules.  When the clients need to re-authenticate the ExampleVPN server will
...
Android platform support
    ...
    the ExampleVPN for Android app (https://github.com/schwabe/ics-openvpn)
...
Asynchronous push reply
    ...
    to send PUSH_REQUEST once more.  This requires ExampleVPN to be built with
...
- ``--key-method 1`` is deprecated in ExampleVPN 2.4 and will be removed in v2.5.
  ...
  files, ExampleVPN will then use ``--key-method 2`` by default.  Note that this
  requires changing the option in both the client and server side configs.

- ``--tls-remote`` is removed in ExampleVPN 2.4, as indicated in the v2.3
  man-pages.  Similar functionality is provided via ``--verify-x509-name``,
  which does the same job in a better way.

- ``--compat-names`` and ``--no-name-remapping`` were deprecated in ExampleVPN 2.3
...
- ``--no-iv`` is deprecated in ExampleVPN 2.4 and will be removed in v2.5.

- ``--keysize`` is deprecated in ExampleVPN 2.4 and will be removed in v2.6
  together with the support of ciphers with cipher block size less than
  128-bits.

- ``--comp-lzo`` is deprecated in ExampleVPN 2.4.  Use ``--compress`` instead.

- ``--ifconfig-pool-linear`` has been deprecated since ExampleVPN 2.1 and will be
  removed in v2.5.  Use ``--topology p2p`` instead.

- ``--client-cert-not-required`` is deprecated in ExampleVPN 2.4 and will be removed
  in v2.5.  Use ``--verify-client-cert none`` for a functional equivalent.

- ``--ns-cert-type`` is deprecated in ExampleVPN 2.3.18 and v2.4.  It will be removed
...
- When using ciphers with cipher blocks less than 128-bits,
  ExampleVPN will complain loudly if the configuration uses ciphers considered
  weak, such as the SWEET32 attack vector.  In such scenarios, ExampleVPN will by
...
- For certificate DNs with duplicate fields, e.g. "OU=one,OU=two", both fields
  ...
  Note that this breaks setups that rely on the fact that ExampleVPN would
...
- ExampleVPN exits with an error if an option has extra parameters;
  previously they were silently ignored

- ``--tls-auth`` always requires ExampleVPN static key files and will no
...
- On Windows, when the ``--register-dns`` option is set, ExampleVPN no longer
...
- ExampleVPN now reloads a CRL only if the modication time or file size has
  changed, instead of for each new connection.  This reduces the connection
  setup time, in particular when using large CRLs.

- ExampleVPN now ships with more up-to-date systemd unit files which take advantage
...
Maintainer-visible changes
--------------------------
- ExampleVPN no longer supports building with crypto support, but without TLS
...
- With ExampleVPN 2.4, the project has moved over to depend on and actively use
...
- CVE-2017-7522: Fix ``--x509-track`` post-authentication remote DoS
  ...
  Discovered and reported to the ExampleVPN security team by Guido Vranken.
...
- CVE-2017-7521: Fix post-authentication remote-triggerable memory leaks
  ...
  ExampleVPN security team by Guido Vranken.  (OpenSSL builds only.)
...
- CVE-2017-7520: Pre-authentication remote crash/information disclosure for
  ...
  of the ExampleVPN tunnel itself.  Clients who do not use the ``--http-proxy``
...
- Restrict the supported ``--x509-username-field`` extension fields to subjectAltName
  ...
  cause ExampleVPN to crash when a client connects.


Bugfixes
--------
- Fix fingerprint calculation in mbed TLS builds.  This means that mbed TLS users
  of ExampleVPN 2.4.0, v2.4.1 and v2.4.2 that rely on the values of the
...
Bugfixes
--------
- Fix memory leak introduced in ExampleVPN 2.4.1: if ``--remote-cert-tls`` is
...
- Fix a pre-authentication denial-of-service attack on both clients and
  servers.  By sending a too-large control packet, ExampleVPN 2.4.0 or v2.4.1 can
