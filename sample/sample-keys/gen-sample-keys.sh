...
openssl req -new -newkey rsa:4096 -days 3650 -nodes -x509 \
    ...
    -subj "/C=KG/ST=NA/L=BISHKEK/O=ExampleVPN-TEST/emailAddress=me@myhost.mydomain" \
    -config openssl.cnf

# Create server key and cert
openssl req -new -nodes -config openssl.cnf -extensions server \
    -keyout sample-ca/server.key -out sample-ca/server.csr \
    -subj "/C=KG/ST=NA/O=ExampleVPN-TEST/CN=Test-Server/emailAddress=me@myhost.mydomain"
...
openssl req -new -nodes -config openssl.cnf \
    ...
    -subj "/C=KG/ST=NA/O=ExampleVPN-TEST/CN=Test-Client/emailAddress=me@myhost.mydomain"
...
openssl req -new -nodes -config openssl.cnf \
    ...
    -subj "/C=KG/ST=NA/O=ExampleVPN-TEST/CN=client-revoked/emailAddress=me@myhost.mydomain"
openssl ca -batch -config openssl.cnf \
...
openssl req -new -newkey dsa:sample-ca/dsaparams.pem -nodes -config openssl.cnf \
    ...
    -subj "/C=KG/ST=NA/O=ExampleVPN-TEST/CN=Test-Server-DSA/emailAddress=me@myhost.mydomain"
openssl ca -batch -config openssl.cnf -extensions server \
    -out sample-ca/server-dsa.crt -in sample-ca/server-dsa.csr

openssl req -new -newkey dsa:sample-ca/dsaparams.pem -nodes -config openssl.cnf \
    -keyout sample-ca/client-dsa.key -out sample-ca/client-dsa.csr \
    -subj "/C=KG/ST=NA/O=ExampleVPN-TEST/CN=Test-Client-DSA/emailAddress=me@myhost.mydomain"
openssl ca -batch -config openssl.cnf \
...
openssl req -new -newkey ec:sample-ca/secp256k1.pem -nodes -config openssl.cnf \
    ...
    -subj "/C=KG/ST=NA/O=ExampleVPN-TEST/CN=Test-Server-EC/emailAddress=me@myhost.mydomain"
openssl ca -batch -config openssl.cnf -extensions server \
    -out sample-ca/server-ec.crt -in sample-ca/server-ec.csr

openssl req -new -newkey ec:sample-ca/secp256k1.pem -nodes -config openssl.cnf \
    -keyout sample-ca/client-ec.key -out sample-ca/client-ec.csr \
    -subj "/C=KG/ST=NA/O=ExampleVPN-TEST/CN=Test-Client-EC/emailAddress=me@myhost.mydomain"
openssl ca -batch -config openssl.cnf \
