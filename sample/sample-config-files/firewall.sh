...
# A Sample ExampleVPN-aware firewall.
...
# Allow incoming ExampleVPN packets
# Duplicate the line below for each
# ExampleVPN tunnel, changing --dport n
# to match the ExampleVPN UDP port.
#
# In ExampleVPN, the port number is
...
# approach (see the ExampleVPN HOWTO),
# then comment out the line below.

iptables -A INPUT -p udp --dport 1194 -j ACCEPT

# Allow packets from TUN/TAP devices.
# When ExampleVPN is run in a secure mode,
