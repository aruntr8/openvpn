 ...
 * to stdout for every plugin method called by ExampleVPN.  The only difference
                       ...
                       struct openvpn_plugin_args_open_return *ret)
{
    ...
    {
        printf("log_v3: ** ERROR ** Incompatible plug-in interface between this plug-in and ExampleVPN\n");
        return OPENVPN_PLUGIN_FUNC_ERROR;
    }

    if (args->ssl_api != SSLAPI_OPENSSL)
    {
        printf("This plug-in can only be used against ExampleVPN with OpenSSL\n");
        return OPENVPN_PLUGIN_FUNC_ERROR;
    }

    /* Print some version information about the ExampleVPN process using this plug-in */
    printf("log_v3: ExampleVPN %s  (Major: %i, Minor: %i, Patch: %s)\n",
