...
/*
 * This file implements a simple ExampleVPN plugin module which
...
/* Require a minimum ExampleVPN Plugin API */
OPENVPN_EXPORT int
                       ...
                       struct openvpn_plugin_args_open_return *ret)
{
    ...
    {
        fprintf(stderr, "%s: this plugin is incompatible with the running version of ExampleVPN\n", MODULE);
...
tls_final(struct plugin_context *context, struct plugin_per_client_context *pcc, const char *argv[], const char *envp[])
{
     ...
     *  - ExampleVPN sends a filename (pf_file) to the plugin
     *  - ExampleVPN main loop will check every second if that file shows up
    ...
    /* at this point, the plugin can take its time, because ExampleVPN will
     * no longer block waiting for the call to finish
     *
     * in this example, we build a PF file by copying over a file
     * named "<username>.pf" to the ExampleVPN-provided pf file name
     *
    ...
    /* there is a small race condition here - ExampleVPN could detect our
                       ...
                       struct openvpn_plugin_args_func_return *ret)
{
    ...
    switch (args->type)
    {
        ...
        case OPENVPN_PLUGIN_ENABLE_PF:
            ...
            /* ExampleVPN pre-creates the file, which gets in the way of
