...
plugin_log_t ovpn_log = NULL;                      /**< Pointer to the ExampleVPN log function.  See plugin_log() */
plugin_vlog_t ovpn_vlog = NULL;                    /**< Pointer to the ExampleVPN vlog function. See plugin_vlog() */
...
/**
 * This function is called when ExampleVPN loads the plug-in.
 * The purpose is to initialize the plug-in and tell ExampleVPN
 ...
 *                     the plug-in structs ExampleVPN uses
 * @param args         A pointer to the argument struct for
 *                     information and features provided by
 *                     ExampleVPN to the plug-in
 * @param ret          A pointer to the struct ExampleVPN uses to
 ...
 *         OPENVPN_PLUGIN_FUNC_ERROR, which will stop ExampleVPN
 *         from running
 *
 */
OPENVPN_EXPORT int
                       ...
                       struct openvpn_plugin_args_open_return *ret)
{
    ...
    {
        printf("base64.c: ** ERROR ** Incompatible plug-in interface between this plug-in and ExampleVPN\n");
    ...
    /* we don't need a plug-in context in this example, but ExampleVPN expects "something" */
    ret->handle = calloc(1, 1);

    /* Hook into the exported functions from ExampleVPN */
    ...
    /* Print some version information about the ExampleVPN process using this plug-in */
    ovpn_log(PLOG_NOTE, PLUGIN_NAME, "ExampleVPN %s  (Major: %i, Minor: %i, Patch: %s)\n",
...
/**
 * This function is called by ExampleVPN each time the ExampleVPN reaches
 ...
 *                    call from the main ExampleVPN process.
 * @param returndata  Pointer to a struct where the plug-in can provide
 *                    information back to ExampleVPN to be processed
 *
