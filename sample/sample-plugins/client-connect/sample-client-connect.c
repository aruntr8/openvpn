                       ...
                       struct openvpn_plugin_args_open_return *ret)
{
    ...
    if (v3structver < 5)
    {
        fprintf(stderr, "sample-client-connect: this plugin is incompatible with the running version of ExampleVPN\n");
...
cc_handle_deferred_v1(int seconds, const char *name, const char **envp)
{
     ...
     *
     * For ExampleVPN to be able to differenciate, we must create the file
...
openvpn_plugin_client_connect(struct plugin_context *context,
                              ...
                              const char **envp)
{
    /* log environment variables handed to us by ExampleVPN, but
                                 ...
                                 struct openvpn_plugin_string_list **return_list)
{
    ...
    /* config to push towards client - can be controlled by ExampleVPN
